####  Muv Box Controller v 0.8 alpha ####

## Uses muvbox.py version 0.9 or greater

# Optimized imports for reducing executable file size
# Legends added
...
# TODO: limitar a frequência de corte a fs/2


from PyQt5 import uic
from PyQt5.QtCore import QTimer, Qt
from PyQt5.QtWidgets import QMainWindow, QApplication, QStyleFactory, QAction, QPushButton, QPlainTextEdit, QTableWidget, QSpinBox, QListWidget, QRadioButton, QLineEdit, QFrame, QLabel, QTableWidgetItem, QVBoxLayout, QMenu, QAction, QInputDialog, QComboBox, QStackedWidget, QMessageBox
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ## ------- If run in Python code: -------
        self.log_filename = './log/' + self.log_date + '_log.txt'
        if os.path.exists('./log') != True:
           os.mkdir('./log')
        self.TABLE_FILE = "./res/ip.txt"
        self.LOGO_FILE = "./res/muv.png"
        self.IP_FILE = "./res/ip.txt"
        
        ## ------- If run for one-file executable: -------
        # fname = './log/' + self.log_date + '_log.txt'
        # self.log_filename = os.path.join(os.path.dirname(sys.executable), fname)
        # logname = os.path.join(os.path.dirname(sys.executable), './log')
        # if os.path.exists(logname) != True:
        #     os.mkdir(logname)
        # self.TABLE_FILE = os.path.join(os.path.dirname(sys.executable), "./res/ip.txt")
        # self.LOGO_FILE = os.path.join(os.path.dirname(sys.executable), "./res/muv.svg")
        # self.IP_FILE = os.path.join(os.path.dirname(sys.executable), "./res/ip.txt")

        #Load the UI Page
        uic.loadUi('main.ui', self)
        self.button = self.findChild(QPushButton, 'connectButton')
        self.pages = self.findChild(QStackedWidget, 'stackedWidget')
        self.start_all_button  = self.findChild(QPushButton, 'pushButton')
        self.connect_all_button = self.findChild(QPushButton, 'pushButton_2')
        self.disconnect_all_button = self.findChild(QPushButton, 'pushButton_8')
        self.save_table_button = self.findChild(QPushButton, 'pushButton_3')
        self.config_button = self.findChild(QPushButton, 'pushButton_4')
        self.update_graph_button = self.findChild(QPushButton, 'pushButton_5')
        self.save_data_button = self.findChild(QPushButton, 'pushButton_6')
        self.sincronizar_button = self.findChild(QPushButton, 'pushButton_7')
        self.stop_all_button = self.findChild(QPushButton, 'pushButton_9')
        self.back_button = self.findChild(QPushButton, 'pushButton_10')
        self.first_config_button = self.findChild(QPushButton, 'pushButton_11')
        self.layout2 = self.findChild(QVBoxLayout, 'verticalLayout_2')
        self.logbox = self.findChild(QPlainTextEdit, 'plainTextEdit')
        self.parar = False  # para a leitura de dados dos sensores se True
        self.time_window = self.findChild(QSpinBox, 'spinBox_4')  # Janela de tempo do gráfico em tempo real, em segundos
        self.radio_button_acc = self.findChild(QRadioButton, 'radioButton')
        self.radio_button_gyr = self.findChild(QRadioButton, 'radioButton_2')
        self.radio_button_angles = self.findChild(QRadioButton, 'radioButton_3')
        self.table = self.findChild(QTableWidget, 'tableWidget')
        self.start_value = self.findChild(QLineEdit, 'lineEdit')
        self.stop_value = self.findChild(QLineEdit, 'lineEdit_2')
        self.main_frame = self.findChild(QFrame, 'frame')
        self.logo_muv = self.findChild(QLabel, 'label_8')
        self.cutoff = self.findChild(QSpinBox, 'spinBox_5')
        self.log_data = 0
        ## Config. Inicial var
        self.scanButton = self.findChild(QPushButton, 'pushButton_12')
        self.saveButton = self.findChild(QPushButton, 'pushButton_15')
        self.cancelButton = self.findChild(QPushButton, 'pushButton_16')
        self.configButton = self.findChild(QPushButton, 'pushButton_13')
        self.backButton = self.findChild(QPushButton, 'pushButton_14')
        self.listview = self.findChild(QListWidget, 'listWidget')
        self.networkComboBox = self.findChild(QComboBox, 'comboBox')
        self.hostnameField = self.findChild(QLineEdit, 'lineEdit_3')
        self.ssidField = self.findChild(QLineEdit, 'lineEdit_4')
        self.passwordField = self.findChild(QLineEdit, 'lineEdit_5')
        self.currentNetworkLabel = self.findChild(QLabel, 'label_9')
        self.infoLabel = self.findChild(QLabel, 'label_14')
        self.infoScanLabel = self.findChild(QLabel, 'label_15')
        self.closeButton = self.findChild(QPushButton, 'pushButton_18')
        ...
        self.threads = []     
        logo = QPixmap(self.LOGO_FILE)
        self.logo_muv.setPixmap(logo.scaled(150, 150, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def start(self):              
        ...
        self.line1.clear()
        self.line2.clear()
        self.ax2[0].lines.clear()
        self.ax2[1].lines.clear()
        self.ax2[2].lines.clear()
        for m in range(len(self.M)):
            # TODO: arrumar as cores dos gráficos
            # Cores das linhas dos gráficos
            style.use('default')
            color_c = 'C' + str(m+1)
            legend = self.M[m].location + ' (' + self.M[m].name + ')'
            ...
            self.line0[m].set_label(legend)
            self.ax2[0].draw_artist(self.line0[m])
            self.ax2[1].draw_artist(self.line1[m])
            self.ax2[2].draw_artist(self.line2[m])

            self.ax2[0].legend(handles=self.line0, loc='upper left', fontsize = 'x-small')

            self.M[m].ACCSCALE = self.table.cellWidget(4, m).currentIndex()
            self.M[m].GYROSCALE = self.table.cellWidget(3,m).currentIndex()
            self.M[m].ACQ_FREQ = int(self.table.item(5,m).text())
            self.M[m].start()
        self.timer2.start()
        self.canvas2.draw()
    ...
    def connect_n(self):
        ...
        self.log_write('['+str(datetime.now())+']:  ' + 'Connecting...') 
        ...
        for column in range(self.table.columnCount()):
            ...
            try:
                self.M[m].location = self.table.item(1,m).text()
            except:
                self.M[m].location = ''
    ...
    def start_log(self):
        try:
            with open(self.log_filename, 'a') as file_object:
                file_object.write('Log started at '+str(datetime.now())+'\n\n')
            self.logbox.setReadOnly(True)
        except:
            self.log_write('['+str(datetime.now())+']:  ' + 'Erro de inicialização do arquivo de log.')

    def update_logbox(self):
        try:
            for m in range(len(self.stringlist0)):
                for n in range(len(self.stringlist0[m])):
                    self.log_write('['+str(datetime.now())+']:  '+self.stringlist0[m][n])
                self.stringlist0[m].clear()   
        except:
            self.log_write('['+str(datetime.now())+']:  ' + 'Erro atualizando logboxes')
    ...
    def log_write(self, string):
        try:
            ...
            self.log_write('['+str(datetime.now())+']:  ' + 'Erro ao tentar escrever no arquivo de log.')        
    ...
    def update_sensor_data(self, opt:int=1):
        ...
        try: 
            ...
            for m in range(nro_active_muvboxes):
                if ((self.M[m].visible and self.M[m].sensors.size>30 and self.M[m].status == 'Running') or opt==0):
                    ...
                    if self.radio_button_acc.isChecked():
                        try:
                            ...
                            self.log_write('['+str(datetime.now())+']:  ' + 'Error ploting accelerometer data. MuvBox ' + str(m))
                    ...
                    if self.radio_button_gyr.isChecked():
                        try:
                            ...
                            self.log_write('['+str(datetime.now())+']:  ' +  'Error ploting gyroscope data.')
                    ...
                    if self.radio_button_angles.isChecked():
                        try:
                            ...
                            self.log_write('['+str(datetime.now())+']:  ' + 'Error ploting angle data.')
            ...
            self.log_write('['+str(datetime.now())+']:  ' + 'Error plotting data - MuvBox ' + str(m) )
    ...
    def include_table(self, event):
        ...
        self.table.setItem(5, self.new_col_num-1, item) 
        ...
        self.table.setCellWidget(3, self.new_col_num - 1, self.gyro_combo)    
        self.table.setCellWidget(4, self.new_col_num - 1, self.acc_combo)
    ...
    def delete_table(self, event):
        if (Mbox('Warning', 'Are you sure you want to erase instance #' + str(self.table.currentColumn() + 1) + '?', 'warning') == True):
            ...
            #self.disconnect()  # Disconnect all MuvBoxes
            #time.sleep(5) # Wait all MuvBoxes disconnect
            del self.M[self.table.currentColumn()]
            self.ax2[0].lines.remove(self.line0[self.table.currentColumn()])
            self.ax2[1].lines.remove(self.line1[self.table.currentColumn()])
            self.ax2[2].lines.remove(self.line2[self.table.currentColumn()])
            del self.line0[self.table.currentColumn()]
            del self.line1[self.table.currentColumn()]
            del self.line2[self.table.currentColumn()]
    ...
    def save_table(self):
        with open(self.TABLE_FILE, 'w', newline='', encoding='iso-8859-1') as stream:
    ...
    def load_table(self):
        if os.path.isfile(self.IP_FILE) != True:
            with open(self.IP_FILE, 'a') as file_object:
                file_object.write('')
        with open(self.TABLE_FILE, 'r', newline='' ,encoding='iso-8859-1') as stream:
            ...
            self.table.setVerticalHeaderLabels(["Name", "Location", "Status", "Gyro. Scale (º/s)", "Acc. Scale (G)", "Nom. acq. rate (pps)", "Inst. acq. rate (pps)", "Bat. level (%)"])
            ...
            # Apaga o status, acq. rate e bat level (colunas 2, 6, e 7)
            for column in range(self.table.columnCount()):
                #item = QTableWidgetItem('')  
                #self.table.setItem(4, column, item)
                item2 = QTableWidgetItem('')  
                self.table.setItem(2, column, item2)
                item3 = QTableWidgetItem('')  
                self.table.setItem(6, column, item3)
                item4 = QTableWidgetItem('')  
                self.table.setItem(7, column, item4)
                if self.table.item(5, column).text == '':
                    self.table.item(5,column).setText('500')     
        ...
        if (len(self.M) >= self.table.columnCount()):
            for column in range(self.table.columnCount()):
                ...
                if self.M[column].sensors.size > 0:
                    ...
                    self.table.setItem(7, column, item)  # battery level
                # update row color
    ...
    def save_data(self):
        ...
        if ok:
            ...
            status = SaveRoutine().start(self.M, comment, start, stop)
            for m in range(len(self.M)):
                self.log_write('['+str(datetime.now())+']:  ' + status[m])
    ...
    def scan(self):
        ...
        if USER_OS == 'Windows':
            ...
            self.log_write('['+str(datetime.now())+']:  ' + self.current_network)
        ...
        # Lista todas as redes wifi ao alcance
            ...
            self.log_write('['+str(datetime.now())+']:  ' + str(apList))
    ...
    def configureNewMuvBox(self):
        try:
            if self.listview.currentRow() >=0:
                ...
                self.log_write('['+str(datetime.now())+']:  ' + muvboxHostname)
                ...
                self.log_write('['+str(datetime.now())+']:  ' + msg)
    ...
    def save(self):
        ...
        try:
            ...
            self.log_write('['+str(datetime.now())+']:  ' + str(e))
            self.infoLabel.setText('Erro conectando a MuvBox! Tente novamente.')
            self.log_write('['+str(datetime.now())+']:  ' + 'Error netsh(Windows) or nmcli(Linux)')
        time.sleep(2)
        ...
        try:
            ...
            self.log_write('['+str(datetime.now())+']:  ' + 'Timeout!')
            self.infoLabel.setText('Erro conectando a MuvBox! Tente novamente.')
        ...
        try:
            ...
            self.log_write('['+str(datetime.now())+']:  ' + 'Erro retornando para rede original!')
            self.infoLabel.setText('MuvBox configurada com sucesso! Clique no botão fechar no canto superior direito da tela.')
